...
const sendSelfWhatsApp=document.getElementById('sendSelfWhatsApp');
...
if(sendSelfWhatsApp) sendSelfWhatsApp.addEventListener('click',async()=>{
  if(!lastSuccessData||!lastSuccessResult) return;
  const msg=buildRegistrationMessage(lastSuccessData,lastSuccessResult);
  const number=normalizeIndiaWhatsApp(lastSuccessData.mobile);
  if(number){ openWhatsApp(number,msg); setShareStatus(t('whatsappOpen')); return; }
  try{
    if(navigator.share){ await navigator.share({title:'MiCasaa Ganesh Utsav Registration',text:msg}); }
    else { await copyText(msg); setShareStatus(t('copied')); }
  }catch(err){ if(err && err.name!=='AbortError') setShareStatus(t('shareUnavailable')); }
});
if(copyConfirmation) copyConfirmation.addEventListener('click',async()=>{
  if(!lastSuccessData||!lastSuccessResult) return;
  try{ await copyText(buildRegistrationMessage(lastSuccessData,lastSuccessResult)); setShareStatus(t('copied')); }
  catch(_){ setShareStatus(t('shareUnavailable')); }
});
